...
const submitConsent = async () => {
  ...
  checkboxes.forEach(function (checkbox) {
    if ((checkbox as HTMLInputElement).checked) {
      consents.push({ category: (checkbox as HTMLInputElement).name });
    }
  });
  ...
  try {
    ...
    await fetchy("/api/consentrecords", "POST", {
      body: { time, description, service_id_str, data_recipient_str, consents },
    });
  } catch {
    return;
  }
...
<template>
  ...
  <span>TurboTax will store the following pieces of data for 2 years: </span>
